...
const SOURCE_ANNOTATED_LESSONS = {
  ...
  "part3/01_rnn_intuition": {
    title: "来源标注版：RNN 到底记住了什么",
    lead: "这篇把 RNN 从“循环网络有记忆”这句口号，改写成“隐藏状态如何一步步压缩历史、为什么长序列会困难、LSTM/GRU 为什么会被发明”。来源标符用于校对递推公式、PyTorch 循环层接口和门控模型的历史位置。",
    sources: [
      { marker: "S1", sourceId: "D2L", note: "RNN 的序列建模直觉、隐藏状态和训练问题" },
      { marker: "S2", sourceId: "PYTORCH_RNN", note: "RNN/LSTM/GRU 工程接口、输入输出形状和层参数" },
      { marker: "S3", sourceId: "LSTM1997", note: "长期依赖和 LSTM 门控记忆的历史动机" },
      { marker: "S4", sourceId: "CHO2014", note: "GRU 与 encoder-decoder 序列建模背景" },
    ],
    sections: [
      {
        title: "一、序列数据：不能把每个时刻当成孤立样本",
        paragraphs: [
          { text: "句子、音频、日志和传感器曲线都有顺序。第 t 个输入不仅要看自己，还要看前面发生过什么；否则“我不喜欢这部电影”和“我喜欢这部电影”就可能被模型看得太像。", refs: ["S1"] },
          { text: "页面里的横向时间步就是序列顺序。每个节点不是一张独立图片，而是一个时间片；箭头表示信息从前一刻流向下一刻。", refs: ["S1"] },
          { text: "如果把序列打乱，RNN 的核心优势会消失，因为它依赖同一套递推规则沿时间轴更新状态。", refs: ["S1", "S2"] },
        ],
        formula: "序列输入：x_1, x_2, ..., x_T",
        prompt: "操作建议：先调大“序列长度”。观察页面里时间步变多后，最早输入的影响是否还能到达最后输出。",
      },
      {
        title: "二、隐藏状态：RNN 的记忆不是仓库，而是压缩摘要",
        paragraphs: [
          { text: "隐藏状态 h_t 可以理解成模型读到第 t 步时的当前笔记。它不是把历史逐字保存，而是把历史压缩成一个固定长度向量。", refs: ["S1", "S2"] },
          { text: "页面中的状态柱表示隐藏状态不同维度的强弱。柱子变化越大，说明当前输入正在明显改写模型的内部摘要。", refs: ["S1"] },
          { text: "隐藏维度越大，理论上能存的信息越多，但也更容易增加参数量、训练时间和过拟合风险。它不是越大越稳。", refs: ["S2"] },
        ],
        formula: "h_t = f(x_t, h_{t-1})",
        prompt: "操作建议：把“隐藏维度”从小调到大，盯住状态柱和最终输出。思考：维度变多后，信息更清楚了，还是只是更难解释了？",
      },
      {
        title: "三、一步递推：当前输入和旧记忆一起决定新记忆",
        paragraphs: [
          { text: "最基本的 RNN 单元会把当前输入 x_t 和上一时刻隐藏状态 h_{t-1} 分别线性变换，再加起来过非线性函数，得到新的 h_t。", refs: ["S1", "S2"] },
          { text: "W_xh 负责读当前输入，W_hh 负责读旧记忆。页面里的循环箭头对应 W_hh 这条路径：同一套循环权重会在每个时间步重复使用。", refs: ["S1", "S2"] },
          { text: "这也解释了 RNN 为什么能处理不同长度的序列：它不是为每个位置单独造一套参数，而是把同一个更新规则重复应用。", refs: ["S1"] },
        ],
        formula: "h_t = tanh(W_xh x_t + W_hh h_{t-1} + b)",
        prompt: "操作建议：调整“记忆保留率”。如果保留率低，早期状态会很快被新输入盖住；如果保留率高，旧信息会更久地影响输出。",
      },
      {
        title: "四、为什么长序列难：梯度也要沿时间反复穿过同一条路",
        paragraphs: [
          { text: "RNN 训练时，误差信号要从后面的时间步反传回前面的时间步。序列越长，梯度要穿过的递推链越长。", refs: ["S1", "S3"] },
          { text: "如果每一步都让梯度缩小一点，连乘很多次后就会接近 0，早期输入几乎学不到；如果每一步都放大一点，梯度又可能爆炸。", refs: ["S1", "S3"] },
          { text: "页面里的长序列演示只是低维直觉，真实训练里还要看梯度范数、截断反向传播长度、初始化和梯度裁剪。", refs: ["S1", "S2"] },
        ],
        formula: "长程梯度 ≈ 多个时间步 Jacobian 的连乘",
        prompt: "操作建议：把“序列长度”调到较大，再加入“输入噪声”。观察早期 token 的影响是否变得更模糊。",
      },
      {
        title: "五、LSTM 和 GRU：给记忆加门，不是简单换个名字",
        paragraphs: [
          { text: "LSTM 的历史动机是缓解长期依赖问题：它用门控机制控制哪些信息保留、哪些信息写入、哪些信息输出。", refs: ["S3"] },
          { text: "GRU 把门控结构做得更紧凑，常用更新门和重置门控制状态变化。它不是“更低级的 LSTM”，而是在参数量和表达能力之间做另一种取舍。", refs: ["S2", "S4"] },
          { text: "所以看页面里的 RNN 直觉时要记住：普通 RNN 是入口，门控模型是为了让记忆流更可控，不是为了让公式更复杂。", refs: ["S1", "S3", "S4"] },
        ],
        formula: "门控直觉：新状态 = 保留旧信息 + 写入新信息",
        prompt: "操作建议：看完本页后跳到“隐藏状态”或“序列模型”，对比普通 RNN、LSTM、GRU 的状态更新方式。重点问：门到底在保护哪一段信息？",
      },
    ],
    pitfalls: [
      "误区 1：RNN 会记住全部历史。正确理解：它只把历史压缩进有限维隐藏状态。",
      "误区 2：隐藏维度越大越好。维度大可能增强容量，也可能带来慢训练和过拟合。",
      "误区 3：长序列只要多训练就能学会。梯度消失/爆炸会让早期信息很难被有效更新。",
