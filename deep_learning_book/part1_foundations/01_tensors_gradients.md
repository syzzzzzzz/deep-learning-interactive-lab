...
向量 (1维张量):  [1, 2, 3, 4]
矩阵 (2维张量):  [[1,2],[3,4]]
3D张量:         图像 [C, H, W] = [3, 224, 224]
4D张量:         批次图像 [B, C, H, W] = [32, 3, 224, 224]
```
...
梯度告诉我们：**如果稍微改变某个参数，损失函数会怎么变化？**

```
梯度 = 损失对参数的偏导数
梯度方向 = 损失增加最快的方向
负梯度方向 = 损失减小最快的方向（我们想要的！）
```
...
```
前向传播：
  x → [乘以w] → z = wx → [加b] → a = z+b → [sigmoid] → y = σ(a) → [MSE] → L
...
| 概念 | 直觉理解 | 关键 API |
|------|----------|----------|
| 张量 | 多维数组 | `torch.tensor`, `torch.randn` |
| 梯度 | 损失对参数的敏感度 | `requires_grad=True`, `.backward()` |
