# 第二章：激活函数、初始化、归一化、过拟合

---

## 2.1 激活函数：给网络注入非线性

没有激活函数，无论多少层线性变换叠加，结果仍然是线性变换。
...
─────────────────────────────────────────────────────────
隐藏层（默认）          ReLU           计算快，不饱和
隐藏层（深层网络）      GELU / Swish   更平滑，性能更好
输出层（二分类）        Sigmoid        输出 [0,1] 概率
输出层（多分类）        Softmax        输出概率分布
输出层（回归）          无（线性）     输出任意实数
RNN/LSTM 门控           Sigmoid/Tanh   需要 [0,1] 或 [-1,1]
Transformer             GELU           GPT/BERT 的选择
```
...
| ReLU | 非线性，避免梯度消失 | 无参数 |
| GELU | 更平滑的非线性 | 无参数 |
| Xavier 初始化 | 保持信号方差（Sigmoid/Tanh） | `nn.init.xavier_normal_` |
| He 初始化 | 保持信号方差（ReLU） | `nn.init.kaiming_normal_` |
| BatchNorm | 稳定训练，加速收敛 | `momentum`, `eps` |
| LayerNorm | 不依赖 batch size | `normalized_shape` |
| Dropout | 防止过拟合 | `p`（丢弃概率） |
| L2 正则化 | 防止权重过大 | `weight_decay` |
