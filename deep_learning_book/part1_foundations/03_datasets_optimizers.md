...
| 优化器 | 特点 | 适用场景 |
|--------|------|----------|
| SGD | 简单，需要调学习率 | 图像分类（配合 momentum） |
| Adam | 自适应学习率，收敛快 | 大多数任务的默认选择 |
| AdamW | Adam + 权重衰减修正 | Transformer 训练 |
| RMSprop | 自适应，适合 RNN | 循环网络 |

| 损失函数 | 任务类型 | 输出层 |
|----------|----------|--------|
| MSELoss | 回归 | 线性 |
| BCELoss | 二分类 | Sigmoid |
| CrossEntropyLoss | 多分类 | 无（内含 Softmax） |
| NLLLoss | 语言模型 | LogSoftmax |
