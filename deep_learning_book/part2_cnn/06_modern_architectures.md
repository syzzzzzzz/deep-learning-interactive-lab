# 现代 CNN 架构：MobileNet / DenseNet / EfficientNet

## 1. 架构演进：从深到广到轻

```
2014  VGGNet        ← 堆叠 3×3 卷积，参数 138M
2015  ResNet        ← 残差连接，深度突破 152 层
2017  DenseNet      ← 密集连接，特征复用
2017  MobileNet     ← 深度可分离卷积，轻量化 4M 参数
2019  EfficientNet  ← 复合缩放，精度-效率 Pareto 最优
```
...
1. Depthwise: 每个通道单独卷积
   输入 [C_in, H, W] × 卷积核 [C_in, 1, K, K] → 中间 [C_in, H, W]
   计算量: C_in × K × K × H × W

2. Pointwise: 1×1 卷积混合通道
   中间 [C_in, H, W] × 卷积核 [C_out, C_in, 1, 1] → 输出 [C_out, H, W]
   计算量: C_out × C_in × H × W

总计算量: C_in × K × K × H × W + C_out × C_in × H × W
计算比: (1/C_out + 1/K²) ≈ 1/K²（当 C_out >> 1 时）
对于 K=3: 标准卷积的 ~1/9 计算量！
```
...
```
ResNet:  x → [Conv→BN→ReLU→Conv→BN] → (+x) → y
         跳跃连接：加法

DenseNet: x → [BN→ReLU→Conv→BN→ReLU→Conv] → concat([x, F(x)]) → y
          密集连接：拼接（通道维度）

每一层的输入 = 之前所有层的输出拼接
L 层网络有 L(L+1)/2 个直接连接
```
...
```
传统缩放方法（只缩放一个维度）：
- ResNet-18/34/50/101/152  ← 只增加深度
- WideResNet               ← 只增加宽度
- ResNeXt                  ← 增加分辨率

问题：单独缩放任一维度，收益递减

EfficientNet 核心思想：复合缩放
同时缩放深度 d、宽度 w、分辨率 r
d = α^φ, w = β^φ, r = γ^φ
约束: α · β² · γ² ≈ 2  (FLOPs 约束)
...
| 架构 | 年份 | 核心创新 | 参数量 | 适用场景 |
|------|------|----------|--------|----------|
| MobileNet V1 | 2017 | 深度可分离卷积 | ~4M | 移动端部署 |
| MobileNet V2 | 2018 | 倒残差 + 线性瓶颈 | ~3.5M | 移动端部署 |
| DenseNet | 2018 | 密集连接，特征复用 | ~8M | 特征复用，少样本 |
| EfficientNet | 2019 | 复合缩放 + SE | ~5-66M | 精度-效率最优 |
