...
`HyperparamSearch`（见 `02_gradient_monitor.md`）提供了网格搜索和随机搜索。
本节补充：
- **学习率范围测试**（LR Range Test / LR Finder）
- **热身 + 余弦退火**调度策略对比
- **超参敏感性分析**：哪个超参数影响最大？

---

## 9.3.1 学习率范围测试（LR Finder）
...
| 工具 | 用途 | 关键输出 |
|------|------|----------|
| LRFinder | 自动找最优学习率范围 | loss 下降最快处的 lr |
| compare_lr_schedules | 对比各种 LR 调度策略 | 各策略的 lr 曲线 |
| hyperparam_sensitivity_analysis | 找出影响最大的超参数 | 敏感性排名 + 散点图 |
