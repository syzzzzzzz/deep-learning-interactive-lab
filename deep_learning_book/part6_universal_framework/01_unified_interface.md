...
model, history = demo_unified_interface()
```
...
| 组件 | 作用 | 关键特性 |
|------|------|----------|
| TensorDatasetWrapper | 统一数据包装 | 自动归一化、一步分割、一步获取 Loader |
| TrainableMixin | 为模型添加训练方法 | `.fit()` 含早停/调度/梯度裁剪 |
| MLP | 预置全连接网络 | 支持 BN、Dropout、多种激活 |
| SimpleCNN | 预置卷积网络 | 适用于图像分类 |
