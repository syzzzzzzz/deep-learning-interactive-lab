...
print("万能框架已准备好！取消注释最后一行来运行完整流水线。")
print("\n使用方法：")
...
| 部分 | 内容 | 核心工具 |
|------|------|----------|
| Part 1 | 张量、梯度、反向传播、激活函数、优化器 | PyTorch 基础 |
| Part 2 | CNN、卷积可视化、LeNet/VGG/ResNet、MNIST | 特征图可视化 |
| Part 3 | RNN/LSTM/GRU、梯度消失、序列预测 | 隐藏状态可视化 |
| Part 4 | Attention、Multi-Head、Transformer、MiniGPT | 注意力权重可视化 |
| Part 5 | 梯度监控、训练动态、超参搜索 | 调试工具箱 |
| Part 6 | 统一接口、万能训练器、完整流水线 | UniversalTrainer |
