# 可扩展插件系统：添加新模型、新数据集、新任务

## 1. 插件系统设计理念

```
核心原则：开闭原则（Open-Closed Principle）
- 对扩展开放：添加新功能不需要改已有代码
- 对修改关闭：已有代码稳定不变

实现方式：
1. 装饰器注册：新组件用装饰器自动注册
2. 配置驱动：YAML/JSON 配置指定使用哪些组件
3. 工厂模式：根据配置动态创建对象
4. 钩子系统：在训练流程的关键点插入自定义逻辑
```
...
| 机制 | 作用 | 关键 API |
|------|------|----------|
| 模型注册表 | 管理所有可用模型 | `@register_model`, `build_model()` |
| 数据集注册表 | 管理所有数据集 | `@register_dataset`, `build_dataset()` |
| 任务注册表 | 管理损失+指标 | `@register_task`, `build_task()` |
| 钩子系统 | 训练流程扩展点 | `register()`, `fire()` |
| 配置驱动 | YAML 指定实验 | 一份配置 = 一个实验 |
| 插件发现 | 自动加载新组件 | `auto_discover_plugins()` |
