...
卷积层的输出叫**特征图（Feature Map）**。
每个卷积核扫描输入，产生一张"响应图"——高亮的地方说明该核在那里"看到了"它想找的模式。
...
| 工具 | 功能 | 关键 API |
|------|------|----------|
| RealTimeFeatureViewer | 实时捕获并显示任意层特征图 | `register_all_conv_layers()`, `show()` |
| show_single_layer | 单通道详细分析（分布+强度） | `show_single_layer(layer, x, ch)` |
| plot_filter_response_heatmap | 激活热力图叠加原图 | `top_k` 最活跃通道 |
| compare_filters_before_after | 训练前后卷积核对比 | 直接读 `.weight` |
