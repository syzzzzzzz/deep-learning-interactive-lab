# 第六章：经典 CNN 架构——LeNet / AlexNet / VGG / ResNet
...
```
1998  LeNet-5        ← 第一个实用 CNN，手写数字识别
2012  AlexNet        ← ImageNet 冠军，深度学习爆发点
2014  VGGNet         ← 极简设计：只用 3×3 卷积堆叠
2015  ResNet         ← 残差连接，解决梯度消失，深度突破 100 层
```

每个架构都解决了前一个的核心问题。我们用简化版实现，保留核心思想。
...
| 架构 | 年份 | 核心创新 | 参数量（简化版） |
|------|------|----------|-----------------|
| LeNet-5 | 1998 | 第一个实用 CNN，Tanh+AvgPool | ~60K |
| AlexNet | 2012 | ReLU+Dropout+GPU，深度学习爆发 | ~200K |
| VGGNet | 2014 | 只用 3×3，极简堆叠 | ~300K |
| ResNet | 2015 | 残差连接，梯度高速公路 | ~170K |
