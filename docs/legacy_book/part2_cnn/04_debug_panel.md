...
| 工具 | 功能 | 关键参数 |
|------|------|----------|
| KernelDebugPanel | 10种经典核效果对比 | `show_all_kernels()` |
| show_kernel_detail | 单核详细分析（分布+统计） | `kernel_name` |
| compare_padding_stride | padding/stride 效果对比 | 4种配置自动对比 |
| LayerByLayerViewer | 逐层特征图流水线 | `show_pipeline(x)` |
