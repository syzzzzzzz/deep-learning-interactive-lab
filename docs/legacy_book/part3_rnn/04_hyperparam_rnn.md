...
| 超参数 | 典型范围 | 影响 |
|--------|----------|------|
| hidden_size | 32 ~ 512 | 记忆容量，越大越强但越慢 |
| num_layers | 1 ~ 4 | 层次抽象能力 |
| dropout | 0 ~ 0.5 | 防过拟合（层间 dropout） |
| learning_rate | 1e-4 ~ 1e-2 | 收敛速度 |
| sequence_length | 10 ~ 200 | 能看多远的历史 |
| bidirectional | True/False | 双向 = 同时看前后文 |
...
res1, res2, res3 = demo_rnn_hyperparams()
```
...
| 超参数 | 建议起点 | 调大效果 | 调小效果 |
|--------|----------|----------|----------|
| hidden_size | 64 | 容量↑，速度↓ | 容量↓，速度↑ |
| num_layers | 1~2 | 抽象层次↑，易过拟合 | 简单快速 |
| dropout | 0.1~0.3 | 正则化↑ | 可能过拟合 |
| bidirectional | False | 性能↑，参数×2 | 参数少，适合实时 |
