# Seq2Seq 与注意力机制：从编码器-解码器到 Bahdanau/Luong 注意力
...
输入序列 "我 爱 深 度 学 习"
    ↓
编码器 LSTM → 上下文向量 c（压缩整个输入）
    ↓
解码器 LSTM → 输出序列 "I love deep learning"
...
1. 计算解码器隐藏状态 h_t 与每个编码器输出 h_s 的对齐分数：
   e_{t,s} = v^T · tanh(W_1 · h_s + W_2 · h_t)
...
```
Bahdanau（加性）: score = v^T · tanh(W1·h_s + W2·h_t)
Luong（乘性）:    score = h_t^T · W · h_s

Luong 更简单，计算更快
也是 Transformer 注意力的直接前身
```
...
RNN Seq2Seq (2014)
    └─ 问题：固定长度上下文向量是瓶颈
    └─ 解决：Bahdanau 注意力（加性，每个解码步动态关注）
         └─ Luong 注意力（乘性，更高效）
              └─ 去掉 RNN，只用注意力 → Transformer (2017)
                   └─ Self-Attention: Q, K, V 都是同一输入
                   └─ Multi-Head: 多组 Q, K, V 并行
                   └─ 位置编码替代循环结构

核心洞察：
注意力机制本身才是关键，RNN 只是载体
去掉 RNN 后，注意力变得更纯粹、更并行
这就是 Transformer 的诞生
```
...
| 组件 | 公式 | 特点 |
|------|------|------|
| 编码器 | h_s = LSTM(x_s) | 双向可选 |
| 解码器（无注意力）| y_t = LSTM(x_t, c) | c 是固定瓶颈 |
| Bahdanau 注意力 | score = v·tanh(W1·h_s + W2·h_t) | 加性，灵活 |
| Luong 注意力 | score = h_t·W·h_s | 乘性，高效 |
| Transformer 注意力 | score = Q·K^T / √d_k | 无 RNN，纯注意力 |
