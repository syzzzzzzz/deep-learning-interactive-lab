# 第三章：位置编码——数学原理、完整代码与可视化

## 3.1 为什么需要位置编码？

Transformer 的自注意力是置换不变的（permutation invariant）：
打乱输入序列的顺序，注意力权重会重新排列，但模型无法区分"我爱你"和"你爱我"。
位置编码（Positional Encoding）给每个位置注入唯一的位置信息。
...
为什么用正弦/余弦？
1. 有界：值域 [-1, 1]，不会影响嵌入的数值范围
2. 唯一性：每个位置的编码向量唯一
3. 相对位置：PE(pos+k) 可以表示为 PE(pos) 的线性变换
   - sin(a+b) = sin(a)cos(b) + cos(a)sin(b)
   - 这意味着模型可以学习相对位置关系
4. 外推性：可以处理训练时未见过的更长序列

频率分析：
- 低维度（i小）：高频，区分相邻位置
- 高维度（i大）：低频，区分远距离位置
- 类比二进制计数：低位快速变化，高位缓慢变化
...
| 特性 | 正弦编码 | 可学习编码 |
|------|----------|------------|
| 参数量 | 0 | max_len x d_model |
| 外推性 | 可以（理论上） | 不能（超出训练长度） |
| 效果 | 略差 | 略好（任务相关） |
| 适用 | 长序列、推理时变长 | 固定长度任务（BERT） |
| 初始化 | 固定公式 | 随机或正弦初始化 |
