# 第八章：极简可运行 Transformer 实现

---

## 8.1 Transformer 整体架构
...
## 8.2 完整 Transformer 实现（可运行、可调试、可看注意力）
...
| 组件 | 作用 | 关键设计 |
|------|------|----------|
| Encoder Block | 理解输入序列 | Self-Attention + FFN |
| Decoder Block | 生成输出序列 | Masked Self-Attn + Cross-Attn + FFN |
| Add & Norm | 稳定训练 | 残差连接 + LayerNorm |
| FFN | 非线性变换 | 维度先扩大4倍再压缩 |
| MiniGPT | 语言模型 | 仅 Decoder，自回归生成 |
