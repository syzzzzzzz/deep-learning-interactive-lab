...
| 工具 | 监控目标 | 诊断意义 |
|------|----------|----------|
| DistributionTracker | 权重均值/标准差/百分位 | 权重是否漂移、坍塌 |
| activation_saturation | ReLU死亡率/Tanh饱和率 | 激活函数是否失效 |
| UpdateRatioMonitor | lr×‖grad‖/‖weight‖ | 学习率是否合适（理想~1e-3） |
