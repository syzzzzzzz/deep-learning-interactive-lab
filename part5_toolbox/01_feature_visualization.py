try:
    """
    自动生成自: part5_toolbox\01_feature_visualization.md
    可独立运行的 Python 源码
    """
    ...
    def print_learning_guide():
        print("""
    学习导读：特征可视化不是“看漂亮图片”，而是在检查模型内部到底学到了什么。

    1. 特征图怎么看
       - conv1 的每个小图是一条通道，亮区表示该通道对某种边缘、纹理或局部形状反应强。
       - conv2 通道更深，通常会组合多种浅层边缘；如果所有通道都像随机噪声，先检查输入归一化和模型是否训练过。
       - fc1 的柱状图显示全连接神经元的激活大小，直方图显示激活是否大量集中在 0 附近。

    2. 卷积核怎么看
       - 卷积核图展示权重本身，不是模型最终关注区域。
       - 浅层卷积核常像边缘探测器；多通道卷积核只显示一个通道时，要记住真实计算会同时融合所有输入通道。

    3. 激活最大化怎么看
       - 左图是通过梯度上升造出来的输入，右图是优化过程。
       - 如果图像越来越极端但没有稳定纹理，说明该通道可能没有学到清晰可解释的模式，或者正则太弱。

    工程坑案例：
       我见过把特征图直接当 Grad-CAM 热力图解释的事故，结论完全跑偏。特征图只能说明某一层某一通道响应强，
       不能直接证明模型最终分类因果。排查时先确认 layer_name、输入预处理、模型训练状态，再比较多张样本的稳定模式。

    进阶思考：
       同一张输入经过 conv1 和 conv2 后，哪些通道更稀疏？如果一个通道只对噪声亮，它会怎样影响后面的分类器？
    """.strip())
    ...
    render_module_error("part5_toolbox/01_feature_visualization.py", e)
