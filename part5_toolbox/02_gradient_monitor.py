try:
    """
    自动生成自: part5_toolbox\02_gradient_monitor.md
    可独立运行的 Python 源码
    """
    ...
    def print_learning_guide():
        print("""
    学习导读：梯度监控是训练调试的体检表，不是事后装饰图。

    1. 梯度监控面板怎么看
       - 上方折线图看每层梯度随训练步数是否平稳，纵轴是 log scale，轻微波动正常，持续上冲或贴近 0 都危险。
       - 左下条形图看最后一步哪一层异常；红色表示低于消失阈值，橙色表示超过爆炸阈值。
       - 右下热力图看异常在时间上如何出现：整片发红多半是梯度长期消失，突然变黄或变亮常见于学习率过高。

    2. 阈值怎么用
       - mean < 1e-6：多数任务里已经很难有效更新，先怀疑激活饱和、初始化不当或网络过深。
       - max > 100：更新可能不稳定，先降学习率，再加 clip_grad_norm_(model.parameters(), 1.0)。
       - 只看单个 batch 不够，至少观察最近 10 到 50 步的趋势。

    3. 和训练曲线怎么联合诊断
       - loss 发散 + 梯度 max 暴涨：学习率过高、标签异常或 loss 实现错误。
       - loss 不动 + 前层 mean 接近 0：梯度消失，优先查初始化、激活函数、残差/归一化。
       - loss 正常下降但某层长期异常：该层可能被冻结、没有接入计算图，或参数名过滤错了。

    工程坑案例：
       我曾经在一个文本分类项目里只看验证准确率，以为模型欠拟合，连续加大模型；后来梯度图显示 embedding 层长期接近 0，
       根因是 tokenizer 把大部分词都映射成了 UNK。先看梯度，可以少走很多弯路。

    进阶思考：
       如果只有最后一层梯度很大，你会先查标签和 loss，还是先重写模型？如果只有前几层梯度消失，残差连接和归一化各解决什么问题？
    """.strip())
    ...
    render_module_error("part5_toolbox/02_gradient_monitor.py", e)
